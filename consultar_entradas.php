...
// Query SQL
$sql = "SELECT produtocodigo, factura, fornecedor,      
	SUM(quantidadee) AS quantidadeTotal, 
    SUM(custo) AS custoTotal 
    FROM entrada 
    WHERE produtocodigo LIKE '%d%' AND dataEntrada BETWEEN '$datainicial ' AND '$datafinal'
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td >" . $row["produtocodigo"] . "</td>";
        echo "<td>" . $row["quantidadeTotal"] . "</td>";
        echo "<td>" . $row["custoTotal"] . "</td>";
